...
| **源码** | [Requester.cs](https://github.com/cout-m6o/OxGKit/blob/master/Assets/OxGKit/Utilities/Scripts/Runtime/Requester/Requester.cs) |
...
| :--- | :--- | :--- |
| [RequestAudio](#requestaudio) | `SelfRequestAudio` | 请求 AudioClip (可缓存)。 |
| [RequestTexture2D](#requesttexture2d) | `SelfRequestTexture2D` | 请求 Texture2D (可缓存)。 |
| [RequestSprite](#requestsprite) | `SelfRequestSprite` | 请求并创建 Sprite (底层缓存 Texture2D)。 |
| [RequestBytes](#requestbytes) | `SelfRequestBytes` | 请求文件 byte[] (不缓存)。 |
| [RequestText](#requesttext) | `SelfRequestText` | 请求文本内容 (可缓存)。 |
...
| 方法 (static) | Self 实例版本 (new 方式) | 说明 |
| :--- | :--- | :--- |
| [InitARCCacheCapacityFor...](#init-arc) | `SelfInitARCCacheCapacityFor...` | 初始指定资源类型的 **ARC 缓存**容量 (会停用该类型的 LRU 缓存)。 |
| [InitLRUCacheCapacityFor...](#init-lru) | `SelfInitLRUCacheCapacityFor...` | 初始指定资源类型的 **LRU 缓存**容量 (会停用该类型的 ARC 缓存)。 |
| [RemoveFromARCCacheFor...](#remove-arc) | `SelfRemoveFromARCCacheFor...` | 从指定资源类型的 ARC 缓存中移除 URL。 |
| [RemoveFromLRUCacheFor...](#remove-lru) | `SelfRemoveFromLRUCacheFor...` | 从指定资源类型的 LRU 缓存中移除 URL。 |
| [ClearARCCacheCapacityFor...](#clear-arc) | `SelfClearARCCacheCapacityFor...` | 清空指定资源类型的 ARC 缓存。 |
| [ClearLRUCacheCapacityFor...](#clear-lru) | `SelfClearLRUCacheCapacityFor...` | 清空指定资源类型的 LRU 缓存。 |
| [AutoRemoveFromCaches](#autoremovefromcaches) | `SelfAutoRemoveFromCaches` | 搜索所有缓存并移除指定 URL。 |
| [ClearAllCaches](#clearallcaches) | `SelfClearAllCaches` | 清空所有缓存。 |
| [Release](#release) | `SelfRelease` | 清空并释放所有缓存容器。 |
