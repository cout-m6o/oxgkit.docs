...
<Highlight color="#0373fc">Attention</Highlight> The **static** methods listed on this page operate through an **internal lazy singleton**; every method also has a **Self**-prefixed **instance counterpart** (e.g. `SelfRequestTexture2D`), so you can `new Requester()` to create an instance with an **isolated cache scope**.
